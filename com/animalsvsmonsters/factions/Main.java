...
public class Main extends JavaPlugin {
	...
	public void onDisable() {
		for (Team team : TeamManager.getManager().getTeams().values()) {
			Database.get().syncUpdate("UPDATE team_info SET kills = ? AND deaths = ? WHERE team = ?",
					new Object[] { team.getKills(), team.getDeaths(), team.getTeamName() });
		}
	}
	...
	private void registerEvent(Listener listener) {
		try {
			...
			LogUtil.log(LogUtil.LogType.WARN,
					"The event listener '" + listener.toString() + "' was not properly registered!");
		}
	}
	...
	public void registerDatabaseConfig() {
		...
		if (database.fileExists()) {
			if (database.loadFile() && database.getString("sql.serverHost").equalsIgnoreCase("0.0.0.0")) {
				LogUtil.log(LogUtil.LogType.SEVERE,
						"The database information in database.yml. Shutting the server down.");
				Bukkit.shutdown();
			}
			return;
		}
	...
	private void loadLang() {
		...
		try {
			...
			LogUtil.log(LogUtil.LogType.SEVERE,
					"lang.yml failed to save! Please report this to MikeTheDev or @MikesCode");
			e.printStackTrace();
		}
	}

	private void createEnderpearlRecipe() {
		ItemStack enderpearl = new ItemBuilder(Material.ENDER_PEARL).name("§eEnderman's Enderpearl")
				.lore("§dThis special enderpearl can be used by enderman",
						"§7" + getConfiguration().getInt("enderpearl_durability") + "/"
								+ getConfiguration().getInt("enderpearl_durability"))
				.build();
