 ...
 * ******************************************************************************************
 */
public class SetLimboCommand implements CommandExecutor{

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(command.getName().equalsIgnoreCase("setlimbo")){
            if(sender instanceof Player){
                ...
                if(player.hasPermission("avmfactions.admin")){
                    if(args.length == 0){
                        f.loadFile();
                        f.set("Limbo", LocationUtil.serializeLocation(player.getLocation()));
                        f.saveFile();
                        TeamManager.getManager().getTeam("Limbo").setSpawn(player.getLocation());
                        player.sendMessage(Lang.SET_LIMBO.toString());
                        return true;
                    }
